...
async def _scrape_all(urls, nav_url):
    async with PlaywrightPageSource(ready_timeout_ms=1500) as source:
...
def scraped(tmp_path_factory):
    ...
    pages, nav_all, nav_tree_only = asyncio.run(_scrape_all(urls, (root / "nav.html").as_uri()))
