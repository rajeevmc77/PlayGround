...
def _check_grid(table: WebNode, grid: list[dict]) -> None:
    if len(grid) != len(table.children):
        raise GridMismatch(f"{table.citation}: {len(grid)} rendered rows, {len(table.children)} in JSON")
...
def join_layout(
    ...
    for node, page in _nodes_with_page(root, None, page_citations):
        if page is None or page not in layouts:
            continue
...
def location_report(root: WebNode, images: list[WebImage]) -> dict[str, dict[str, int]]:
    ...
    items = [(n.type, n.location) for n in _walk(root) if n.type != "root"]
    items += [("Image", image.location) for image in images]
    for type_, location in items:
