...
async def download_images(images: list[WebImage], source: WebSource, output_dir: str) -> None:
    out_dir = Path(output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    targets = [img for img in images if img.src]
    semaphore = asyncio.Semaphore(DOWNLOAD_CONCURRENCY)
    await asyncio.gather(*(_fetch_and_write(img, source, semaphore, out_dir) for img in targets))


async def _fetch_and_write(
    img: WebImage, source: WebSource, semaphore: asyncio.Semaphore, out_dir: Path
) -> None:
    async with semaphore:
        content = await source.fetch_image(img.src)
